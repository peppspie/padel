...
export function ScoreModal({ match, teamA, teamB, setsToWin, gamesPerSet, onClose, onSave }: ScoreModalProps) {
  ...
  // Calculate max possible sets (e.g. Best of 3 = 3 sets max)
  const maxSets = (setsToWin * 2) - 1;

  // Initialize with existing score or create empty sets
  const [sets, setSets] = useState<SetScore[]>(() => {
    let currentSets = match.score.sets.length > 0 
      ? [...match.score.sets] 
      : [];
  ...
  const getLimit = (opponentScore: number) => {
    // If opponent is close to winning (gamesPerSet - 1), we can go up to gamesPerSet + 1 (tie-break or win by 2)
    // Otherwise, standard limit is gamesPerSet
    if (opponentScore >= gamesPerSet - 1) {
      return gamesPerSet + 1;
    }
    return gamesPerSet;
  };
  ...
  const handleSave = () => {
    // Basic validation logic for sets won
    let setsWonA = 0;
    let setsWonB = 0;
              ...
              <div key={index} className="bg-gray-900/50 p-3 rounded-xl border border-gray-700">
                ...
                <div className="flex items-center justify-center gap-6">
                  ...
                  <div className="flex items-center gap-1">
                    <button 
                      onClick={() => handleIncrement(index, 'teamA', -1)}
                      className="w-8 h-8 flex items-center justify-center bg-gray-800 text-red-400 rounded-lg hover:bg-gray-700 font-bold active:bg-gray-600 touch-manipulation"
                    >
                      -
                    </button>
                    <input
                      type="number"
                      value={set.teamA}
                      onChange={(e) => updateScore(index, 'teamA', parseInt(e.target.value) || 0)}
                      className="w-12 h-12 text-center bg-gray-800 border border-gray-600 rounded-lg text-white font-bold text-xl focus:border-green-500 outline-none"
                    />
                    <button 
                      onClick={() => handleIncrement(index, 'teamA', 1)}
                      className="w-8 h-8 flex items-center justify-center bg-gray-800 text-green-400 rounded-lg hover:bg-gray-700 font-bold active:bg-gray-600 touch-manipulation"
                    >
                      +
                    </button>
                  </div>
                  ...
                  <div className="flex items-center gap-1">
                    <button 
                      onClick={() => handleIncrement(index, 'teamB', -1)}
                      className="w-8 h-8 flex items-center justify-center bg-gray-800 text-red-400 rounded-lg hover:bg-gray-700 font-bold active:bg-gray-600 touch-manipulation"
                    >
                      -
                    </button>
                    <input
                      type="number"
                      value={set.teamB}
                      onChange={(e) => updateScore(index, 'teamB', parseInt(e.target.value) || 0)}
                      className="w-12 h-12 text-center bg-gray-800 border border-gray-600 rounded-lg text-white font-bold text-xl focus:border-blue-500 outline-none"
                    />
                    <button 
                      onClick={() => handleIncrement(index, 'teamB', 1)}
                      className="w-8 h-8 flex items-center justify-center bg-gray-800 text-blue-400 rounded-lg hover:bg-gray-700 font-bold active:bg-gray-600 touch-manipulation"
                    >
                      +
                    </button>
                  </div>
